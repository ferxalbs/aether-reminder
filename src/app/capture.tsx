...
export default function CaptureRoute() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const nextEnvelope = external ?? createCaptureEnvelope({
        ...
        try {
          ...
          if (!(cause instanceof CaptureError) || cause.category !== 'domain_validation') throw cause;
          nextDraft = { title: '', priority: 'medium', sources: sourcesFrom(nextEnvelope) };
        }
        if (!cancelled) {
  ...
  const image = useMemo(
    () => draft?.sources.find((source): source is Extract<CaptureSource, { kind: 'image' }> => source.kind === 'image'),
    [draft?.sources],
  );
  const url = useMemo(
    () => draft?.sources.find((source): source is Extract<CaptureSource, { kind: 'url' }> => source.kind === 'url'),
    [draft?.sources],
  );
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      if (stored) {
        ...
        if (!claimed) throw new CaptureError('database_busy', 'This capture is already being processed.', true);
        try {
          const result = await orchestrator.commit({ ...claimed.envelope, reviewRequired: false }, draft);
          await inbox.markCommitted(stored.id, claimed.claimToken, result.task.id);
        } catch (cause) {
          const category = cause instanceof CaptureError ? cause.category : 'unknown';
          const retryable = cause instanceof CaptureError ? cause.retryable : true;
          await inbox.markFailure(stored.id, claimed.claimToken, category, retryable);
          throw cause;
        }
      ...
      setError(cause instanceof Error ? cause.message : 'Capture could not be saved.');
    } finally {
      setSaving(false);
    }
    ...
    >
      ...
      >
        <View style={styles.content}>
          <Typography variant="headline" accessibilityRole="header">Add to AETHER</Typography>
          <Typography variant="body" color={secondary}>
            Review the capture, then add one reminder.
          </Typography>

          {loading ? <ActivityIndicator accessibilityLabel="Loading capture" /> : null}
          {image ? (
            <Image
              source={{ uri: image.assetRef }}
              contentFit="contain"
              accessibilityLabel={image.displayName ? `Shared image: ${image.displayName}` : 'Shared image preview'}
              style={[styles.image, { backgroundColor: isDark ? Colors.surfaceRaisedDark : Colors.surfaceRaisedLight }]}
            />
          ) : null}
          {url ? (
            <Typography selectable variant="caption" color={secondary} numberOfLines={3}>
              {url.url}
            </Typography>
          ) : null}
          {draft ? (
            ...
            <Typography accessibilityRole="alert" color={secondary}>{error}</Typography>
          ) : null}
          {draft?.dueDate ? (
            <Typography variant="caption" color={secondary}>
              {draft.dueDate}{draft.dueTime ? ` · ${draft.dueTime}` : ''}
            </Typography>
          ) : null}
          <View style={styles.actions}>
            <Button
              ...
              fullWidth
            />
            <Button label="Cancel" onPress={discard} variant="ghost" disabled={saving} fullWidth />
          </View>
        </View>
      </ScrollView>
    </KeyboardAvoidingView>
