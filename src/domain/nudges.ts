...
export function createEmptyNudgeProfile(now = new Date().toISOString()): NudgeProfile {
  return {
...
export function sanitizeNudgeProfile(value: unknown): NudgeProfile {
  ...
  const completionAverage = finiteNumber(record.completionDelayAverageMinutes, NaN);
  const adaptiveCompletionAverage = finiteNumber(record.adaptiveCompletionDelayAverageMinutes, NaN);
  return {
    ...base,
    policyVersion: typeof record.policyVersion === 'string' ? record.policyVersion : base.policyVersion,
    sampleCount,
    ...
    repeatedDeferrals: Math.max(0, Math.floor(finiteNumber(record.repeatedDeferrals))),
    adaptiveCompletionCount: Math.max(0, Math.floor(finiteNumber(record.adaptiveCompletionCount))),
    adaptiveDeferralCount: Math.max(0, Math.floor(finiteNumber(record.adaptiveDeferralCount))),
    manualRescheduleCount: Math.max(0, Math.floor(finiteNumber(record.manualRescheduleCount))),
    completionDelayAverageMinutes: Number.isFinite(completionAverage)
      ? bounded(completionAverage, -24 * 60, 7 * 24 * 60)
      : null,
    adaptiveCompletionDelayAverageMinutes: Number.isFinite(adaptiveCompletionAverage)
...
function ewma(previous: number | null, next: number, alpha = 0.35): number {
  return bounded(previous === null ? next : previous + alpha * (next - previous), 0, NUDGE_SNOOZE_MAX_MINUTES);
}

/** Apply one compact explicit event to the aggregate profile. */
export function applyNudgeEvent(profileInput: NudgeProfile, event: NudgeEvent): NudgeProfile {
  ...
  if (event.eventType === 'task_completed' || event.eventType === 'notification_action_complete') {
    ...
    if (event.numericValue !== null && Number.isFinite(event.numericValue)) {
      ...
      next.completionDelayAverageMinutes = next.completionDelayAverageMinutes === null
        ? delay
        : next.completionDelayAverageMinutes + 0.25 * (delay - next.completionDelayAverageMinutes);
    }
    if (event.secondaryNumericValue !== null
      ...
      next.adaptiveCompletionDelayAverageMinutes = next.adaptiveCompletionDelayAverageMinutes === null
        ? delay
  ...
  if (event.eventType === 'notification_action_snooze') {
    ...
    if (event.nudgeId || event.source === 'adaptive_nudge_action') next.adaptiveDeferralCount += 1;
  }
...
export function preferredNudgeDelayMinutes(profileInput: NudgeProfile): number | null {
  const profile = sanitizeNudgeProfile(profileInput);
  if (profile.snoozeSamples < NUDGE_MINIMUM_SAMPLES || profile.snoozeAverageMinutes === null) return null;
...
export function preferredNudgeTimeBucket(profileInput: NudgeProfile): NudgeTimeBucket | null {
  const profile = sanitizeNudgeProfile(profileInput);
...
export function isReminderAdaptive(reminder: Reminder | null | undefined): boolean {
