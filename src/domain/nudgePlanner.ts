...
function effectiveTimezone(
  task: Task,
  settings: NudgePlannerSettings,
): string | null {
  if (task.dueSemantics === "fixed") {
    return (
      task.dueTimezone ?? settings.deviceTimezone ?? getDeviceTimeZone() ?? null
    );
  }
...
function localDateTimeToDate(
  ...
  if (!Number.isFinite(value.getTime()))
    throw new Error("Invalid local nudge date.");
  return value;
}
...
function nextPreferredBucketStart(
  ...
  let next = localDateTimeToDate(
    date,
    PREFERRED_BUCKET_START[preferredBucket],
    timezone,
  );
  if (next <= candidate) {
    date = addLocalCalendarDays(date, 1);
    next = localDateTimeToDate(
      date,
      PREFERRED_BUCKET_START[preferredBucket],
      timezone,
    );
...
function activeAdaptiveNudge(state: NudgeReminderState): boolean {
  return state.adaptiveNudges.some(
    (nudge) => nudge.enabled && !nudge.cancelledAt && !nudge.consumedAt,
  );
}

function negativeTimingFeedback(
  profile: NudgeProfile,
  bucket: NudgeTimeBucket,
): boolean {
...
function result(
  ...
  return proposal
    ? { status, reason, confidence: profile.confidence, proposal }
    : {
        status,
        reason,
...
export function planNudge(
  ...
  const settings: NudgePlannerSettings = {
    ...
    maxAdaptiveNudgesPerTaskPerDay: Math.max(
      1,
      Math.floor(settingsInput.maxAdaptiveNudgesPerTaskPerDay),
    ),
    maxAdaptiveNudgesPerDay: Math.max(
      1,
      Math.floor(settingsInput.maxAdaptiveNudgesPerDay),
    ),
    cooldownMinutes: Math.max(0, settingsInput.cooldownMinutes),
    recoveryHandoffMinutes: Math.max(
      NUDGE_RECOVERY_HANDOFF_MINUTES,
      settingsInput.recoveryHandoffMinutes,
    ),
  ...
  if (!settings.enabled)
    return result("none", "adaptive_nudges_disabled", profile);
  if (task.completed)
    return result("none", "suppressed_task_complete", profile);
  ...
  if (!isValidLocalDate(task.dueDate))
    return result("none", "invalid_schedule", profile);
  if (task.dueTime !== null && !isValidLocalTime(task.dueTime))
    return result("none", "invalid_schedule", profile);
  if (
    reminderState.lastExplicitDeferralAt &&
    minutesSince(reminderState.lastExplicitDeferralAt, now) <
      settings.cooldownMinutes &&
    !reminderState.adaptiveNudges.some(
      (nudge) =>
    ...
    if (task.dueTime === null) {
      ...
      if (due <= now) {
        ...
        candidate = addMinutes(
          ...
          Math.max(
            NUDGE_MIN_DELAY_MINUTES,
            NUDGE_BASELINE_DELAY_MINUTES - elapsed,
          ),
        );
        if (
          candidate.getTime() >
          addMinutes(due, settings.recoveryHandoffMinutes).getTime()
        ) {
          return result("none", "delegated_to_smart_recovery", profile);
        }
      } else {
        ...
        candidate = addMinutes(
          due,
          learnedDelay ?? NUDGE_BASELINE_DELAY_MINUTES,
        );
      }
    }
  } catch {
  ...
  if (preferredBucket && preferredBucket !== bucket && due && due > now) {
    const preferredCandidate = nextPreferredBucketStart(
      candidate,
      preferredBucket,
      timezone,
    );
  ...
  if (activeAdaptiveNudge(reminderState))
    return result("none", "duplicate_pending_nudge", profile);

  if (
    reminderState.adaptiveNudgesToday >=
      settings.maxAdaptiveNudgesPerTaskPerDay ||
  ...
  if (
    reminderState.lastAdaptiveNudgeAt &&
    minutesSince(reminderState.lastAdaptiveNudgeAt, now) <
      settings.cooldownMinutes
  ) {
...
export const DEFAULT_NUDGE_PLANNER_SETTINGS: Omit<
