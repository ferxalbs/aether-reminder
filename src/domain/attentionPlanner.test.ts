...
function candidate(
  overrides: Partial<AttentionCandidateFacts> = {},
): AttentionCandidateFacts {
  return {
    taskId: "task-1",
    title: "Task 1",
...
describe("AttentionPlanner", () => {
  ...
  test("ambiguous equivalent candidates return choose mode", () => {
    ...
    expect(result.now).toBeNull();
    expect(result.choices.map((item) => item.taskId)).toEqual([
      "alpha",
      "beta",
    ]);
    expect(result.choices.every((item) => item.confidence === "low")).toBe(
      true,
    );
  });
  ...
  test("nudge_due raises a candidate without ranking suppressed nudges", () => {
    ...
    expect(result.next.some((item) => item.taskId === "suppressed")).toBe(
      false,
    );
  });
  ...
  test("hysteresis keeps the current automatic NOW through a small ranking change", () => {
    const first = plan([
      candidate({
        taskId: "current",
        temporal: { ...candidate().temporal, minutesUntilDue: 120 },
      }),
      candidate({
        taskId: "other",
        temporal: { ...candidate().temporal, minutesUntilDue: 130 },
      }),
    ]);
    ...
    const second = plan(
      [
        candidate({
          taskId: "current",
          temporal: { ...candidate().temporal, minutesUntilDue: 100 },
        }),
        candidate({
          taskId: "other",
          temporal: { ...candidate().temporal, minutesUntilDue: 90 },
        }),
      ],
      { previousPlan: first },
    );
  ...
  test("same facts and clock produce the same plan without mutation", () => {
    const input = [
      candidate({ taskId: "b" }),
      candidate({ taskId: "a", priority: "low" }),
    ];
  ...
  test("NEXT remains bounded and has deterministic tie ordering", () => {
    const nextCandidates = Array.from({ length: 8 }, (_, index) =>
      candidate({
    ...
    expect(result.next.map((item) => item.taskId)).toEqual([
      "next-01",
      "next-02",
      "next-03",
      "next-04",
