...
function targetInstant(
  target: NotificationNudgeActionInput["target"],
): Date | null {
  if (!target) return null;
  try {
    const timezone =
      target.semantics === "fixed" ? target.timezone : getDeviceTimeZone();
    if (timezone)
...
function completionDelayMinutes(task: Task): number | null {
  ...
  try {
    ...
    const value =
      (new Date(task.completedAt).getTime() - due.getTime()) / 60_000;
    return Number.isFinite(value)
      ? Math.max(-24 * 60, Math.min(7 * 24 * 60, value))
      : null;
  } catch {
    return null;
  }
...
function completionRelativeToNudgeMinutes(
  task: Task,
  reminder: Reminder,
): number | null {
  if (!task.completedAt || !reminder.scheduledDate || !reminder.scheduledTime)
    return null;
  try {
    const timezone =
      reminder.semantics === "fixed"
    ...
    const value =
      (new Date(task.completedAt).getTime() - nudgedAt.getTime()) / 60_000;
    return Number.isFinite(value)
      ? Math.max(-24 * 60, Math.min(7 * 24 * 60, value))
      : null;
  } catch {
...
function newestReminder(reminders: readonly Reminder[]): Reminder | null {
  return (
    [...reminders].sort((a, b) => b.updatedAt.localeCompare(a.updatedAt))[0] ??
    null
  );
}
...
export class NudgeService {
  ...
  async getAttentionSignals(
    ...
    for (const reminder of nudges) {
      ...
      if (dueAt && dueAt.getTime() <= now.getTime())
        signals.set(reminder.taskId, "nudge_due");
    }
    return signals;
  }
  ...
  private async projectReminders(
  ...
  private async cancelTaskNudges(taskId: string): Promise<void> {
    ...
    await this.projectReminders(
      after.filter((item) => !item.enabled && item.projectionDirty),
    );
  }
  ...
  private async invalidateFloatingNudges(): Promise<void> {
    ...
    for (const reminder of floating)
      await this.reminders.cancelAdaptiveNudge(reminder.id);
    const refreshed = await Promise.all(
      floating.map((item) => this.reminders.getById(item.id)),
    );
    await this.projectReminders(
      refreshed.filter((item): item is Reminder =>
        Boolean(item && item.projectionDirty),
      ),
    );
  }
  ...
  async replanTask(taskId: string, now = new Date()): Promise<NudgePlanResult> {
    ...
    if (!task) {
      ...
      return {
        status: "none",
        reason: "suppressed_task_deleted",
    ...
    const adaptive = reminders.filter(
      (reminder) => reminder.kind === "adaptive_followup",
    );
    const primary =
      reminders.find((reminder) => reminder.kind !== "adaptive_followup") ??
      null;
    const lastDeferral = enabled
      ? await this.events.getLastDeferralForTask(task.id)
      : null;
    const profile = enabled
      ? await this.events.getProfile()
      : createEmptyNudgeProfile();
    const plannerState = (
    ...
    if (plan.status === "proposed" && plan.proposal) {
      const [taskBudget, globalBudget] = await Promise.all([
        this.reminders.countAdaptiveNudgesForDate(
          plan.proposal.scheduledDate,
          task.id,
        ),
  ...
  async replanBoundedHorizon(now = new Date()): Promise<void> {
    ...
    const previousTimezone = await this.appMeta.get(
      ADAPTIVE_NUDGES_TIMEZONE_KEY,
    );
    ...
    const throughDate = addLocalCalendarDays(
      deviceDate,
      DEFAULT_NUDGE_PLANNER_SETTINGS.planningHorizonDays + 1,
    );
  ...
  async recordTaskCompleted(task: Task, source = "manual"): Promise<void> {
    ...
    const nudge = newestReminder(
      adaptive.filter((reminder) => reminder.enabled),
    );
    const timezone =
      task.dueSemantics === "fixed"
        ? (task.dueTimezone ?? getDeviceTimeZone())
    ...
    try {
      await this.events.append(
        buildEvent({
          ...
          secondaryNumericValue: nudge
            ? completionRelativeToNudgeMinutes(task, nudge)
            : null,
  ...
  async recordNotificationAction(
    input: NotificationNudgeActionInput,
  ): Promise<void> {
    const isAdaptive = input.reminder.kind === "adaptive_followup";
    const instant =
      input.action === "snooze" ? targetInstant(input.target) : null;
    const numericValue = instant
      ? Math.max(
          1,
          Math.min(240, (instant.getTime() - input.now.getTime()) / 60_000),
        )
      : null;
    const timezone =
      input.reminder.semantics === "fixed"
  ...
  async recordNotificationOpened(
    input: NotificationNudgeOpenedInput,
  ): Promise<void> {
    const isAdaptive = input.reminder.kind === "adaptive_followup";
    const timezone =
      input.reminder.semantics === "fixed"
  ...
  async diagnostics(): Promise<NudgeDiagnostics> {
    ...
    return {
      ...
      activeAdaptiveNudges: (await this.reminders.listAdaptiveNudges()).filter(
        (item) => item.enabled,
      ).length,
    };
  }
