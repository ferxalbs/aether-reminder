...
describe('recurrence and adaptive nudge safety', () => {
  test('engine-generated nudges are not copied into the next occurrence', async () => {
    ...
    const advanced = await services.recurrence.advanceAfterCompletion(completed.value);
    expect(advanced).not.toBeNull();
    const nextReminders = await repos.reminders.listForTask(advanced!.nextTask.id);
