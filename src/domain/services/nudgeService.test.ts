...
describe('NudgeService', () => {
  test('persists one idempotent nudge and preserves desired state when native projection fails', async () => {
    ...
    const completedNudge = (await repos.reminders.listAdaptiveNudgesForTask(task.id))[0];
  ...
  test('reset clears behavioral history without touching ordinary reminders', async () => {
    ...
    const repos = createRepositories(db);
    const services = createDomainServices(db);
    const task = await repos.tasks.create({ id: 'reset-task', title: 'Keep task' });
  ...
  test('rescheduling a task cancels its old nudge before replanning the new schedule', async () => {
    ...
    await services.nudges.replanTask(task.id, new Date('2030-01-02T08:00:00.000Z'));
    const before = (await repos.reminders.listAdaptiveNudgesForTask(task.id))[0];
    ...
    expect(after.some((item) => item.enabled && item.scheduledDate === '2030-01-03')).toBe(true);
    await db.closeAsync?.();
  });
});
