...
function plannerSettings(overrides: Partial<NudgePlannerSettings> = {}): NudgePlannerSettings {
  return {
...
function state(overrides: Partial<Parameters<typeof NudgePlanner.plan>[1]> = {}) {
  return {
...
describe('NudgePlanner', () => {
  ...
  test('uses a conservative baseline with insufficient evidence', () => {
    const result = NudgePlanner.plan(task(), state(), createEmptyNudgeProfile(), plannerSettings(), now);
  ...
  test('does not personalize before the minimum sample threshold', () => {
    ...
    for (let index = 0; index < NUDGE_MINIMUM_SAMPLES - 1; index += 1) {
      profile = applyNudgeEvent(profile, event({ id: `event-${index}`, numericValue: 60 }));
    }
    const result = NudgePlanner.plan(task(), state(), profile, plannerSettings(), now);
  ...
  test('learns an explicit snooze delay and clamps outliers', () => {
    ...
    for (let index = 0; index < NUDGE_MINIMUM_SAMPLES; index += 1) {
      profile = applyNudgeEvent(profile, event({ id: `event-${index}`, numericValue: index === 0 ? 9999 : 12 }));
    }
  ...
  test('uses a preferred time bucket only after confident evidence', () => {
    ...
    for (let index = 0; index < 10; index += 1) {
      profile = applyNudgeEvent(profile, event({
        id: `event-${index}`,
  ...
  test('suppresses completed, deleted, and duplicate tasks', () => {
    expect(NudgePlanner.plan(task({ completed: true }), state(), createEmptyNudgeProfile(), plannerSettings(), now).reason)
      .toBe('suppressed_task_complete');
    expect(NudgePlanner.plan(task({ deletedAt: '2030-01-01T00:00:00.000Z' }), state(), createEmptyNudgeProfile(), plannerSettings(), now).reason)
      .toBe('suppressed_task_deleted');
    expect(NudgePlanner.plan(task(), state({ adaptiveNudges: [reminder()] }), createEmptyNudgeProfile(), plannerSettings(), now).reason)
  ...
  test('returns a non-error budget suppression and enforces cooldowns', () => {
    ...
    expect(NudgePlanner.plan(task(), state({ adaptiveNudgesToday: 1 }), profile, plannerSettings(), now).status)
      .toBe('suppressed_by_budget');
    expect(NudgePlanner.plan(task(), state({ globalAdaptiveNudgesToday: 3 }), profile, plannerSettings(), now).reason)
      .toBe('suppressed_daily_budget');
    expect(NudgePlanner.plan(task(), state({ lastAdaptiveNudgeAt: '2030-01-02T07:00:00.000Z' }), profile, plannerSettings(), now).reason)
      .toBe('cooldown');
    expect(NudgePlanner.plan(task(), state({ lastExplicitDeferralAt: '2030-01-02T07:00:00.000Z' }), profile, plannerSettings(), now).reason)
  ...
  test('keeps fixed plans stable while floating plans follow a timezone change', () => {
    const fixedTask = task({ dueSemantics: 'fixed', dueTimezone: 'America/New_York' });
    ...
    expect(fixedLosAngeles.proposal?.scheduledDate).toBe(fixedTokyo.proposal?.scheduledDate);
    expect(fixedLosAngeles.proposal?.scheduledTime).toBe(fixedTokyo.proposal?.scheduledTime);
