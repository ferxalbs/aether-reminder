...
describe("getNextRecurrenceDate", () => {
  test("advances daily intervals without timezone conversion", () => {
    expect(getNextRecurrenceDate(rule({ interval: 2 }), "2026-08-09")).toBe(
      "2026-08-11",
    );
  });
  ...
  test("stops at end date and max occurrence limits", () => {
    expect(
      getNextRecurrenceDate(rule({ endDate: "2026-08-09" }), "2026-08-09"),
    ).toBeNull();
    expect(
      getNextRecurrenceDate(rule({ maxOccurrences: 1 }), "2026-08-09"),
    ).toBeNull();
  });
  ...
  test("resolves fixed occurrences from the series anchor after a current occurrence moved", () => {
    ...
    expect(
      getNextRecurrenceDate(fixed, getRecurrenceOccurrenceDate(fixed, 1)!),
    ).toBe("2026-08-10");
  });
});
