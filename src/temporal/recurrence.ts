...
export function differenceInLocalCalendarDays(a: string, b: string): number {
  return Math.floor(
    (toUtcDate(b).getTime() - toUtcDate(a).getTime()) / 86_400_000,
  );
}
...
function addMonthsFromAnchor(
  ...
  const absoluteMonth =
    base.getUTCFullYear() * 12 + base.getUTCMonth() + monthOffset;
...
function normalizedWeekdays(rule: RecurrenceRule): number[] {
  const values = (rule.weekdays ?? []).filter(
...
function normalizedMonthDays(rule: RecurrenceRule): number[] {
  const values = (rule.monthDays ?? []).filter(
...
export function getNextRecurrenceDate(
  ...
  switch (rule.frequency) {
    ...
    case "weekly": {
      ...
      for (let offset = 1; offset <= 366 * 6; offset += 1) {
        ...
        const weekOffset = Math.floor(
          differenceInLocalCalendarDays(anchorWeek, startOfWeekSunday(next)) /
            7,
        );
    ...
    case "yearly": {
      ...
      for (
        let yearOffset = firstYearOffset;
        yearOffset <= firstYearOffset + 400;
        yearOffset += 1
      ) {
        if (yearOffset % interval !== 0) continue;
        const year = anchor.year + yearOffset;
        const day = clampDay(year, anchor.month, anchor.day);
        const next = formatUtcDate(
          new Date(Date.UTC(year, anchor.month - 1, day)),
        );
        if (next > fromDate) {
          candidate = next;
          break;
