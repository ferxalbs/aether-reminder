...
import type { InferenceProvider, ModelCapabilities } from '@/services/ai/inference';
...
function providerFor(calls: string[], capabilities = agentCapabilities): InferenceProvider {
  return {
    id: 'test-openrouter',
...
describe('deterministic OpenRouter model selection', () => {
  test('validates the exact DeepSeek default instead of choosing a catalog fallback', async () => {
    const calls: string[] = [];
    await expect(resolveAgentModel('', 'or-key', providerFor(calls))).resolves.toBe('deepseek/deepseek-v4-flash');
    expect(calls).toEqual(['deepseek/deepseek-v4-flash']);
  });

  test('preserves an explicit selected model id after validation', async () => {
    const calls: string[] = [];
    await expect(resolveAgentModel('  provider/model  ', 'or-key', providerFor(calls))).resolves.toBe('provider/model');
    expect(calls).toEqual(['provider/model']);
  });

  test('fails clearly when the selected model cannot run the tool agent', async () => {
    const calls: string[] = [];
    const incompatible: ModelCapabilities = { ...agentCapabilities, tools: false, toolChoice: false, compatibility: 'CONVERSATION_ONLY' };
    const result = resolveAgentModel('provider/chat-only', 'or-key', providerFor(calls, incompatible));
