...
const MockActivityIndicator: React.FC<Record<string, unknown>> = (props) =>
  React.createElement('ActivityIndicator', props, props.children as React.ReactNode);
...
const MockFlatList: React.FC<Record<string, unknown>> = (props) => {
  ...
  return React.createElement(
    ...
    data.map((item, index) =>
      React.createElement(
        React.Fragment,
        { key: (props.keyExtractor as (item: unknown) => string)?.(item) ?? index },
...
describe('ModelCatalogSheet native-first presentation', () => {
  ...
  test('filters models by search query', () => {
    ...
    // Should find GPT-4o
    const gptModel = root.findByProps({ accessibilityLabel: 'GPT-4o, OpenAI, 128k context, Agent-Ready' });
    expect(gptModel).toBeDefined();

    // Should not find Claude 3.5 Sonnet in filtered items
    expect(
      root.findAllByProps({ accessibilityLabel: 'Claude 3.5 Sonnet, Anthropic, 200k context, Agent-Ready' }).length,
    ).toBe(0);
  });
  ...
  test('handles error state with error text', () => {
    ...
    const root = renderer!.root;
    const errorNodes = root.findAll((node) =>
      node.props?.children === 'Network error fetching models',
    );
  ...
  test('does not select disabled model without tool support', () => {
    ...
    const llamaModel = root.findByProps({
      accessibilityLabel: 'Llama 3 8B Instruct, Meta, 8k context, No Tool Support',
    });
  ...
  test('triggers onRefresh callback from header action', () => {
    ...
    const root = renderer!.root;
    const refreshBtn = root.findByProps({ accessibilityLabel: 'Force refresh model catalog' });
