...
describe("TaskEditor scheduling presets", () => {
  ...
  test("maps common recurrence rules back to commercial presets", () => {
    ...
    expect(inferRepeatPreset(rule({ weekdays: [1, 2, 3, 4, 5] }))).toBe(
      "weekdays",
    );
    expect(inferRepeatPreset(rule({ weekdays: [3] }))).toBe("weekly");
    expect(
      inferRepeatPreset(
        rule({ frequency: "monthly", weekdays: null, monthDays: [10] }),
      ),
    ).toBe("monthly");
  ...
  test("keeps weekly/monthly presets anchored to a changed date", () => {
    ...
    expect(
      normalizeRecurrenceStateForDate(weekly, "2026-08-12").weekdays,
    ).toEqual([3]);

    const monthly = applyRepeatPreset(initial, "monthly", "2026-08-10");
    expect(
      normalizeRecurrenceStateForDate(monthly, "2026-08-28").monthDays,
    ).toEqual([28]);
  });
