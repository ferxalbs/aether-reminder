...
mock.module('react-native', () => ({
  Platform: {
    ...
    select: (obj: Record<string, unknown>) => obj[currentPlatform] ?? obj.default,
  },
  StyleSheet: {
...
mock.module('@/stores/settings.store', () => ({
  useSettingsStore: (selector?: (state: { hapticsEnabled: boolean; materialColorsEnabled: boolean }) => unknown) => {
    const state = {
...
Object.assign(
  (await import('@/stores/settings.store')).useSettingsStore,
  {
    getState: () => ({
      hapticsEnabled: mockHapticsEnabled,
      materialColorsEnabled: mockMaterialColorsEnabled,
...
describe('ToggleSwitch Platform Native Adapter', () => {
  ...
  describe('Android (Jetpack Compose Switch)', () => {
    test('renders ComposeSwitch with explicit AETHER OLED dark colors when material colors disabled', () => {
      ...
      expect(switchComp.props.modifiers).toEqual([{ type: 'testID', tag: 'haptics-switch' }]);
