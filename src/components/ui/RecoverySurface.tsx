...
function formatSchedule(schedule: RecoverySchedule | null): string {
  ...
  return schedule.dueTime
    ? `${schedule.dueDate} at ${schedule.dueTime}`
    : schedule.dueDate;
}
...
export function RecoverySheet({
  ...
  const [choices, setChoices] = useState<Record<string, RecoveryChoice>>(() =>
    Object.fromEntries(
      plan.proposals.map((proposal) => [proposal.taskId, proposal.proposed]),
    ),
  );
  ...
  const activeCount = useMemo(
    () =>
      Object.values(choices).filter((choice) => typeof choice === "object")
        .length,
    [choices],
  );
  ...
  const apply = async () => {
    ...
    try {
      ...
      if (result.applied.length > 0) {
        ...
        setMessage(
          "These changes are already applied. The plan was refreshed.",
        );
      } else if (result.skippedStale.length > 0) {
        setMessage(
          "Some tasks changed before Apply Recovery. Review the refreshed plan.",
        );
      } else if (result.failed.length > 0) {
        setMessage(
          "Recovery could not be applied. Your tasks were not changed.",
        );
      }
    } finally {
      setApplying(false);
            ...
            >
              ...
              <View style={styles.scheduleRows}>
                <Typography variant="caption">
                  Previous · {formatSchedule(proposal.previous)}
                </Typography>
                <Typography variant="caption">
                  Proposed · {formatSchedule(proposal.proposed)}
                </Typography>
              </View>
              {proposal.recurrence ? (
                <Typography variant="tiny" color={colors.textSecondary}>
              ...
              <View style={styles.alternatives}>
                {proposal.alternatives.map((alternative) => (
                  <Button
                    ...
                    label={
                      alternative.schedule
                        ? `${alternative.label} · ${formatSchedule(alternative.schedule)}`
                        : alternative.label
                    }
                    variant={
                      choiceMatches(choice, alternative)
                        ? "primary"
                        : "secondary"
                    }
                    size="sm"
