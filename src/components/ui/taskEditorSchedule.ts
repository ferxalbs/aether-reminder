...
export type SchedulePreset =
  "today" | "tomorrow" | "next_week" | "custom" | "none";
      ...
      return {
        ...
        weekdays:
          state.frequency === "weekly" && state.weekdays.length === 0
            ...
            : state.weekdays,
        monthDays:
          state.frequency === "monthly" && state.monthDays.length === 0
            ? [parsed.day]
            : state.monthDays,
      };
  }
...
export function normalizeRecurrenceStateForDate(
  ...
  if (state.preset === "weekly")
    return { ...state, weekdays: [parsed.weekday] };
  ...
  const weekdays =
    frequency === "weekly"
      ? state.weekdays.length > 0
        ? [...new Set(state.weekdays)].sort((a, b) => a - b)
        : [parsed.weekday]
      : null;
  const monthDays =
    frequency === "monthly"
      ? state.monthDays.length > 0
        ? [...new Set(state.monthDays)].sort((a, b) => a - b)
        : [parsed.day]
      : null;
  return {
    frequency,
