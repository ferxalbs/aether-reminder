...
describe('initial tier', () => {
  ...
  test('low-RAM device ceilings at reduced', () => {
    const state = createGovernorState(capabilitiesFixture({ lowRamDevice: true }));
  ...
  test('API below native blur does not force minimal', () => {
    const state = createGovernorState(capabilitiesFixture({ androidApiLevel: 30, supportsNativeBlur: false }));
  ...
  test('refresh rate does not select the tier by itself', () => {
    for (const rate of [60, 90, 120, 144]) {
      const state = createGovernorState(capabilitiesFixture({ maximumRefreshRateHz: rate }));
      expect(state.staticCeiling).toBe('full');
    }
  });
});
...
describe('hysteresis', () => {
  ...
  test('required consecutive windows degrade one step', () => {
    ...
    state = { ...state, runtimeTier: 'full' };
    state = applySnapshots(state, jankSnapshots(0.12, MOTION_DOWNGRADE_WINDOWS));
    expect(state.runtimeTier).toBe('standard');
  ...
  test('recovery requires sustained stability', () => {
    let state = createGovernorState(capabilitiesFixture());
    state = applySnapshots(state, jankSnapshots(0.22, MOTION_DOWNGRADE_WINDOWS));
    expect(state.runtimeTier).toBe('reduced');
  ...
  test('ceiling is respected during recovery', () => {
    let state = createGovernorState(capabilitiesFixture({ lowRamDevice: true }));
    state = applySnapshots(state, jankSnapshots(0.01, MOTION_RECOVERY_WINDOWS, { lowRamDevice: true }));
...
describe('thermal', () => {
  test('maps every thermal enum to a ceiling', () => {
    const cases: [NonNullable<Parameters<typeof snapshotFixture>[0]>['thermalState'], string][] = [
      ['nominal', 'full'],
      ['fair', 'standard'],
      ['light', 'standard'],
      ['moderate', 'standard'],
...
describe('power mode', () => {
  ...
  test('disabling low power does not instantly skip recovery hysteresis', () => {
    let state = createGovernorState(capabilitiesFixture());
    state = applySnapshots(state, jankSnapshots(0.22, MOTION_DOWNGRADE_WINDOWS, { lowPowerMode: true }));
    expect(state.runtimeTier).toBe('reduced');
    state = applySnapshots(state, jankSnapshots(0.22, 2, { lowPowerMode: false }));
    expect(state.runtimeTier).toBe('reduced');
  });
});
...
describe('accessibility', () => {
  ...
  test('reduce motion remains after performance improves', () => {
    ...
    state = reduceMotionState(state, { type: 'accessibility', accessibility: reduced });
    state = applySnapshots(state, jankSnapshots(0.0, MOTION_RECOVERY_WINDOWS), reduced);
    expect(state.runtimeTier).toBe('minimal');
...
describe('warmup and empty samples', () => {
  ...
  test('empty frame samples are ignored', () => {
    ...
    state = reduceMotionState(state, {
      type: 'snapshot',
      snapshot: snapshotFixture({ frames: { frameCount: 0, jankCount: 0, jankRatio: null } }),
      accessibility: a11y,
    });
    expect(state.runtimeTier).toBe('full');
  });
