...
export class AetherCommandExecutor {
  ...
  private async replanAdaptiveNudges(
    taskId: string,
    reason: string,
  ): Promise<void> {
    try {
  ...
  private async syncDueReminder(
    ...
    const beforeReminder = beforeTask
      ? await this.findDueReminder(beforeTask)
      : null;
  ...
  async applyRecovery(
    ...
    const uniqueSelections = [
      ...new Map(
        selections.map((selection) => [selection.proposal.taskId, selection]),
      ).values(),
    ];
    const selected = uniqueSelections.filter(
      (selection) => selection.schedule !== null,
    );
    result.excluded.push(
      ...uniqueSelections
        .filter((selection) => selection.schedule === null)
        .map((selection) => selection.proposal.taskId),
    );
    ...
    for (const outcome of outcomes) {
      const selection = selected.find(
        (item) => item.proposal.taskId === outcome.taskId,
      );
    ...
    for (const item of applied) {
      ...
      try {
        await this.syncDueReminder(item.before, item.after, "recovery", {
          project: false,
        });
      } catch (error) {
        result.projectionFailures.push({
          ...
          message:
            error instanceof Error
              ? error.message
              : "Reminder projection repair failed.",
        });
      }
      try {
        const reconciliation = await this.syncTaskProjections(
          item.after.id,
          "recovery",
        );
  ...
  async undoRecovery(
    receipt: import("@/domain/receipts").ActionReceipt,
  ): Promise<RecoveryApplyResult> {
    const items = getRecoveryUndoItems(receipt);
    if (!items)
      throw new Error(
        "Recovery undo payload is invalid or no longer available.",
      );
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Recovery Undo failed.";
      result.failed.push(
        ...items.map((item) => ({ taskId: item.taskId, message })),
      );
      return result;
    }
    ...
    for (const outcome of outcomes) {
      const item = items.find(
        (candidate) => candidate.taskId === outcome.taskId,
      );
      if (!item) continue;
      if (outcome.applied && outcome.before && outcome.after) {
        result.applied.push(outcome.taskId);
        try {
          await this.syncDueReminder(
            outcome.before,
            outcome.after,
            "recovery_undo",
            { project: false },
  ...
  async createTask(input: CreateTaskInput, source = "manual") {
    ...
    try {
      ...
      await this.services.tasks
        .deleteTask(result.value.id, "command_rollback")
        .catch(() => undefined);
      throw error;
    }
  }
  ...
  async updateTask(id: string, input: UpdateTaskInput, source = "manual") {
    ...
    try {
      ...
      const scheduleChanged =
        before.dueDate !== result.value.dueDate ||
      ...
      if (scheduleChanged) {
        try {
          await this.services.nudges.recordTaskRescheduled(
            result.value,
            source,
          );
        } catch (error) {
  ...
  async createTaskEditorState(
    input: CreateTaskEditorStateInput,
    source = "manual",
  ) {
   ...
   */
  async saveTaskEditorState(
    id: string,
    input: SaveTaskEditorStateInput,
    source = "manual",
  ) {
    ...
    const taskResult = await this.services.tasks.updateTask(
      id,
      input.task,
      source,
    );
    ...
    try {
      await this.services.repos.reminders.markTaskDirty(id);
      reminderChange = await this.syncDueReminder(
        beforeTask,
        taskResult.value,
        source,
      );
      ...
      const scheduleChanged =
        beforeTask.dueDate !== taskResult.value.dueDate ||
      ...
      if (input.recurrence && targetDate) {
        ...
        recurrenceResult = await this.services.recurrence.stopRule(
          beforeRule.id,
        );
      }
  ...
  async completeTask(id: string, source = "manual") {
    ...
    const recurrence = await this.services.recurrence.advanceAfterCompletion(
      result.value,
      source,
    );
    ...
    return {
      ...
      receipt: createReceipt({
        ...
        summary:
          `Completed “${result.value.title}” · next ${recurrence.nextTask.dueDate ?? ""}`.trim(),
  ...
  async reopenTask(id: string, source = "manual") {
    if (source === "undo") {
      const recurringUndo =
        await this.services.recurrence.undoLatestCompletionForTask(id);
      if (recurringUndo) {
  ...
  async createRecurringTask(
    ...
    try {
      ...
      await this.services.tasks
        .deleteTask(result.task.id, "command_rollback")
        .catch(() => undefined);
      throw error;
    }
  }
  ...
  async scheduleReminder(input: ScheduleReminderInput, source = "manual") {
    const result = await this.services.reminders.scheduleReminder(
      input,
      source,
    );
  ...
  async rescheduleReminder(
    ...
    if (
      ...
      result.value.kind !== "adaptive_followup" &&
      before
    ) {
