...
describe("Smart Recovery command path", () => {
  test("applies a batch once, reports duplicate apply as already applied, supports Undo, and regenerates from SQLite state", async () => {
    ...
    const second = await createOverdue(
      core,
      "Second slipped",
      overdue,
      "medium",
    ...
    expect(plan.proposals.map((proposal) => proposal.taskId)).toEqual([
      first.value.id,
      second.value.id,
    ]);
    const selections = plan.proposals.map((proposal) => ({
      proposal,
      schedule: proposal.proposed,
    }));
    ...
    expect((await core.services.tasks.getTask(first.value.id))?.dueDate).toBe(
      overdue,
    );
    expect((await core.services.tasks.getTask(second.value.id))?.dueDate).toBe(
      overdue,
    );
    ...
    expect(afterUndo.proposals.map((proposal) => proposal.taskId)).toEqual([
      first.value.id,
      second.value.id,
    ]);
    await db.closeAsync?.();
  ...
  test("skips a stale entry while applying the rest of the batch", async () => {
    ...
    const plan = await core.services.recovery.generatePlan(now);
    const staleProposal = plan.proposals.find(
      (proposal) => proposal.taskId === first.value.id,
    )!;
    await core.commands.updateTask(first.value.id, {
      dueDate: addLocalCalendarDays(today, 1),
    });
  ...
  test("keeps domain recovery successful when reliability projection repair fails", async () => {
    ...
    expect((await core.services.tasks.getTask(created.value.id))?.dueDate).toBe(
      today,
    );
    await db.closeAsync?.();
  });
  ...
  test("fixed recurrence preserves future cadence when the current occurrence is recovered", async () => {
    ...
    const proposal = plan.proposals.find(
      (item) => item.taskId === created.task.id,
    )!;
    expect(proposal.recurrence?.mode).toBe("fixed");
    await core.commands.applyRecovery(plan.id, [
      { proposal, schedule: proposal.proposed },
    ]);

    await core.commands.completeTask(created.task.id);
    const next = await core.services.tasks.getTask(
      "recurrence_fixed-recovery-rule_2",
    );
  ...
  test("after-completion recurrence still anchors from completion date", async () => {
    ...
    const next = await core.services.tasks.getTask(
      "recurrence_completion-recovery-rule_2",
    );
