...
describe("Realtime client-secret authentication", () => {
  test("mints a short-lived secret with the requested transcription session", async () => {
    ...
    globalThis.fetch = (async (_input, init) => {
      ...
      return new Response(
        JSON.stringify({ value: "ek_test", expires_at: 12345 }),
        {
          status: 200,
          headers: {
    ...
    const secret = await new OpenAIByokClientSecretProvider(
      "user-standard-key",
    ...
    expect(session).toEqual(
      buildRealtimeSessionPayload(defaultRealtimeTranscriptionConfig),
    );
  ...
  test("preserves invalid-credential status and provider details without exposing the key", async () => {
    globalThis.fetch = (async () =>
      new Response(
        ...
        { status: 401, headers: { "x-request-id": "req_1" } },
      )) as typeof fetch;
    await expect(
      new OpenAIByokClientSecretProvider("secret-value").create(
  ...
  test("distinguishes tier, account, configuration, and temporary model failures", async () => {
    ...
    for (const [status, code, message, expected] of cases) {
      ...
      await expect(
        new OpenAIByokClientSecretProvider("key").create(
          defaultRealtimeTranscriptionConfig,
        ),
