...
const transitions: Record<
  ...
  readonly RealtimeWebSocketState[]
> = {
  idle: ["connecting"],
...
function providerError(
  ...
  if (!transcriptionFailure) {
    if (
      diagnostic.includes("invalid_api_key") ||
      ...
      diagnostic.includes("temporarily") ||
      diagnostic.includes("server_error") ||
      diagnostic.includes("rate_limit")
    ) {
      voiceCode = "MODEL_TEMPORARILY_UNAVAILABLE";
    } else if (
      diagnostic.includes("session") ||
      diagnostic.includes("configuration") ||
      diagnostic.includes("invalid_request") ||
      diagnostic.includes("unsupported") ||
      diagnostic.includes("invalid_model")
    ) {
      voiceCode = "SESSION_CONFIGURATION_INVALID";
    } else if (
      diagnostic.includes("invalid_event") ||
      diagnostic.includes("protocol")
    ) {
...
function validateTranscriptionConfig(
  ...
  if (
    config.model !== REALTIME_TRANSCRIPTION_MODEL ||
...
export class OpenAIRealtimeWebSocketTransport implements RealtimeTranscriptionTransport {
  ...
  private backpressureDeadlineTimer: ReturnType<typeof setTimeout> | null =
    null;
  ...
  constructor(options: OpenAIRealtimeWebSocketTransportOptions = {}) {
    ...
    if (this.options.packetBytes <= 0 || this.options.packetBytes % 2 !== 0) {
      throw new VoiceError(
        ...
        "Realtime packet size must be a positive even byte count.",
      );
    }
    if (
      this.options.maxQueuedPackets <= 0 ||
      this.options.maxWebSocketBufferedBytes <= 0
    ) {
      throw new VoiceError(
  ...
  private transition(next: RealtimeWebSocketState): void {
    if (!transitions[this.state].includes(next)) {
      throw protocolError(
        `Illegal realtime transport transition: ${this.state} -> ${next}.`,
      );
    }
    this.state = next;
  }
  ...
  private clearTimers(): void {
    ...
    if (this.backpressureDeadlineTimer)
      clearTimeout(this.backpressureDeadlineTimer);
  ...
  private fail(error: VoiceError): void {
    ...
    const configurationPending =
      this.state === "configuring" || Boolean(this.rejectConfigure);
    this.clearTimers();
    this.queue = [];
  ...
  async connect(clientSecret: string): Promise<void> {
    if (this.state !== "idle") {
      throw new VoiceError(
        ...
        "Realtime transport can only connect from idle.",
      );
    }
    ...
    if (!secret)
      throw new VoiceError(
        ...
        "Realtime client secret is empty.",
      );

    this.transition("connecting");
    this.expectedClose = false;
    this.closeDiagnosticEmitted = false;
    this.options.diagnostics?.record("websocket_connecting", {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      try {
        const url =
          this.options.webSocketUrl ?? buildRealtimeTranscriptionWebSocketUrl();
        ...
        socket.onopen = () => {
          ...
          try {
            ...
            this.fail(
              error instanceof VoiceError
                ? error
                : protocolError("Could not enter connected state.", error),
            );
            return;
          }
          if (this.connectTimer) clearTimeout(this.connectTimer);
          ...
          this.resolveConnect = null;
          this.rejectConnect = null;
          this.options.diagnostics?.record("websocket_open", {
            webSocketState: "open",
          });
        ...
        this.fail(
          error instanceof VoiceError
            ...
            : new VoiceError(
                "REALTIME_CONNECT_FAILED",
                "Could not create the realtime WebSocket.",
                { cause: error },
              ),
        );
      }
  ...
  private handleMessage(data: unknown): void {
    try {
      if (typeof data !== "string")
        throw protocolError("Realtime server returned a non-text event.");
      const parsed: unknown = JSON.parse(data);
      const event = asRecord(parsed);
      if (typeof event.type !== "string")
        throw protocolError("Realtime server event has no type.");
      ...
      switch (event.type) {
        case "session.created": {
          if (
            ...
            throw protocolError(
              "Realtime session.created event has no session object.",
            );
          }
          const session = asRecord(event.session);
          if (session.type !== "transcription") {
            throw protocolError(
              "Realtime server created a non-transcription session.",
            );
          }
        ...
        case "session.updated": {
          if (this.state !== "configuring")
            throw protocolError("Unexpected realtime session.updated event.");
          if (
            !event.session ||
            typeof event.session !== "object" ||
            Array.isArray(event.session)
            ...
            throw protocolError(
              "Realtime session.updated event has no session object.",
            );
          }
          const session = asRecord(event.session);
          if (session.type !== "transcription") {
            throw protocolError(
              "Realtime server acknowledged a non-transcription session.",
            );
          }
          if (this.configureTimer) clearTimeout(this.configureTimer);
          this.configureTimer = null;
        ...
        case "conversation.item.input_audio_transcription.delta": {
          if (
            ...
            throw protocolError(
              "Transcript delta arrived before the transcription session was ready.",
            );
          }
          if (
          ...
          if (
            this.transcriptionDeltaCount === 1 ||
            this.transcriptionDeltaCount % 10 === 0
          ) {
  ...
  appendPcm(data: ArrayBuffer): void {
    if (this.state !== "ready" || this.committed) {
      throw new VoiceError(
        ...
        "PCM can only be appended after session configuration.",
      );
    }
    ...
    if (data.byteLength % 2 !== 0) {
      throw new VoiceError(
        "AUDIO_FORMAT_UNSUPPORTED",
        "PCM16 append contains an incomplete sample.",
      );
    }

    const incoming = new Uint8Array(data);
    const combined = new Uint8Array(
      this.partial.byteLength + incoming.byteLength,
    );
    ...
    while (combined.byteLength - offset >= this.options.packetBytes) {
      this.enqueue(
        combined.slice(offset, offset + this.options.packetBytes).buffer,
      );
  ...
  private enqueue(packet: ArrayBuffer): void {
    if (this.queue.length >= this.options.maxQueuedPackets) {
      const error = new VoiceError(
        "REALTIME_BACKPRESSURE",
        "Realtime audio queue limit was exceeded.",
      );
      this.fail(error);
      throw error;
    }
    ...
    if (!this.backpressureDeadlineTimer) {
      this.backpressureDeadlineTimer = setTimeout(() => {
        ...
        this.fail(
          new VoiceError(
            "REALTIME_BACKPRESSURE",
            "Realtime WebSocket could not drain its send buffer.",
          ),
        );
  ...
  private flush(): void {
    if (
      !this.socket ||
      this.socket.readyState !== OPEN_STATE ||
      (this.state !== "ready" && this.state !== "committing")
    )
      return;
    ...
    if (packet) {
      const payload = JSON.stringify({
        ...
        audio: pcm16ToBase64(packet),
      });
      if (
        this.socket.bufferedAmount + payload.length >
        this.options.maxWebSocketBufferedBytes
      ) {
    ...
    if (
      this.socket.bufferedAmount + commitPayload.length >
      this.options.maxWebSocketBufferedBytes
    ) {
      this.waitForBackpressure();
      return;
  ...
  commit(): void {
    ...
    if (
      ...
      throw new VoiceError(
        "REALTIME_PROTOCOL_ERROR",
        "Realtime commit requires a ready WebSocket session.",
      );
    }
    if (
      this.queue.length === 0 &&
      this.partial.byteLength === 0 &&
      this.audioBytesSubmitted === 0
    ) {
      throw new VoiceError(
        "EMPTY_TRANSCRIPT",
        "Realtime commit has no PCM audio.",
      );
    }
    this.committed = true;
  ...
  private finish(clearAudio: boolean): void {
    ...
    this.clearTimers();
    this.rejectPending(
      new VoiceError("CANCELLED", "Realtime transport closed."),
    );
    this.queue = [];
    this.queuedAudioBytes = 0;
    this.partial = new Uint8Array(0);
