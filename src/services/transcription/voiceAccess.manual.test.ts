...
function deterministicPcmFixture(): ArrayBuffer {
  ...
  for (let index = 0; index < samples; index += 1) {
    const sample = Math.round(
      Math.sin((2 * Math.PI * 440 * index) / sampleRate) * 2_000,
    );
...
test.skipIf(!apiKey)(
  "live: gpt-live-transcribe transcription session accepts PCM and completes",
  async () => {
    ...
    expect(nestedTranscriptionModel(session)).toBe(
      REALTIME_TRANSCRIPTION_MODEL,
    );
    expect(
      isTranscriptionWebSocketUrl(buildRealtimeTranscriptionWebSocketUrl()),
    ).toBe(true);
    ...
    const outcomePromise = new Promise<RealtimeTransportEvent>(
      (resolve, reject) => {
        outcomeTimeout = setTimeout(
          () => reject(new Error("timed out waiting for provider completion")),
          45_000,
        );
    ...
    try {
      let secret;
      try {
        secret = await new OpenAIByokClientSecretProvider(apiKey).create(
          minimalRealtimeTranscriptionConfig,
        );
      } catch (error) {
        failAt(
