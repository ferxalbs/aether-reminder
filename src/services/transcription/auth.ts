...
function parseSecret(value: unknown, requestId?: string): RealtimeClientSecret {
  if (!value || typeof value !== "object") {
    throw new VoiceError(
      ...
      "OpenAI returned an invalid client secret.",
    );
  }
...
function accessErrorCode(
  ...
  const diagnostic =
    `${String(error?.code ?? "")} ${String(error?.message ?? "")} ${String(error?.type ?? "")}`.toLowerCase();
  if (
    status === 401 ||
    diagnostic.includes("invalid_api_key") ||
    diagnostic.includes("invalid credential")
  ) {
    return "INVALID_CREDENTIAL";
  }
  if (
    diagnostic.includes("free tier") ||
    diagnostic.includes("usage tier") ||
    diagnostic.includes("tier not supported")
  ) {
...
export class OpenAIByokClientSecretProvider implements RealtimeClientSecretProvider {
  ...
  async create(
    ...
    try {
      ...
      throw new VoiceError(
        "REALTIME_AUTH_FAILED",
        "Could not request an OpenAI client secret.",
        { cause: error },
      );
    }
    if (!response.ok) {
      ...
      throw new VoiceError(
        accessErrorCode(response.status, payload),
    ...
    try {
      ...
      throw new VoiceError(
        "REALTIME_AUTH_FAILED",
        "OpenAI returned invalid client-secret JSON.",
        { cause: error },
      );
    }
  }
...
export async function testOpenAIRealtimeConnection(
  ...
  return {
    provider: "OpenAI",
    connected: true,
    modelAccess: secret.modelAccess,
  };
}

export {
  accessErrorCode as classifyOpenAIModelAccessError,
