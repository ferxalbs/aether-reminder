...
class FakeTransport {
  ...
  appendPcm(data: ArrayBuffer): void { this.appends.push(data); }
  commit(): void { this.commits += 1; }
  cancel(): void { this.cancels += 1; }
...
function harness(overrides: Partial<VoiceSessionDependencies> = {}) {
  ...
  const dependencies: VoiceSessionDependencies = {
    permission: {
      get: async () => ({ granted: true, canAskAgain: true, status: 'granted' }),
      request: async () => ({ granted: true, canAskAgain: true, status: 'granted' }),
    },
    audioSession: {
      activate: async () => { cleanup.push('activate'); },
      deactivate: async () => { cleanup.push('deactivate'); },
    },
    capture,
    clientSecrets: { create: async () => ({ value: 'ephemeral', expiresAt: Date.now() / 1000 + 60, modelAccess: 'MODEL_EXISTS' }) },
    createTransport: () => {
    ...
    onFinalTranscript: async (transcript) => { transcripts.push(transcript); },
    ...overrides,
  };
...
describe('VoiceSession orchestration', () => {
  test('checks permission, activates audio, starts native PCM, and reaches listening', async () => {
    ...
    expect(h.session.snapshot).toMatchObject({ state: 'listening', permission: 'granted', error: null });
  ...
  test('enters explicit permission_denied when permission is permanently blocked', async () => {
    const h = harness({
      permission: {
        get: async () => ({ granted: false, canAskAgain: false, status: 'denied' }),
        request: async () => ({ granted: false, canAskAgain: false, status: 'denied' }),
      },
    });
    await h.session.start();
    expect(h.session.snapshot).toMatchObject({ state: 'permission_denied', permission: 'blocked' });
  ...
  test('streams normalized PCM, reconciles deltas, and commits manually once', async () => {
    ...
    await h.session.start();
    h.capture.emit();
    h.transports[0].emit({ type: 'speechDelta', itemId: 'item-1', delta: 'Remind ' });
    h.transports[0].emit({ type: 'speechDelta', itemId: 'item-1', delta: 'me' });
  ...
  test('classifies an empty completed transcript without invoking parsing', async () => {
    ...
    h.capture.emit();
    await h.session.stop();
    h.transports[0].emit({ type: 'completed', itemId: 'empty', transcript: '   ' });
    await tick();
  ...
  test('retry creates a new session after transient authentication failure', async () => {
    ...
    const h = harness({
      clientSecrets: {
        create: async () => {
          attempts += 1;
          if (attempts === 1) throw new VoiceError('REALTIME_AUTH_FAILED', 'temporary');
          return { value: 'ephemeral', expiresAt: Date.now() / 1000 + 60, modelAccess: 'MODEL_EXISTS' };
        },
      },
    });
  ...
  test('parser rejection enters parsing_failed after final transcript', async () => {
    const h = harness({ onFinalTranscript: async () => { throw new Error('parser unavailable'); } });
    await h.session.start();
    h.capture.emit();
    await h.session.stop();
    h.transports[0].emit({ type: 'completed', itemId: 'item', transcript: 'Create reminder' });
    await tick();
