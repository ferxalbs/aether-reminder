...
describe("Realtime transcription protocol", () => {
  ...
  test("rejects using the transcription model as if it were a WebSocket URL", () => {
    expect(() =>
      buildRealtimeTranscriptionWebSocketUrl(REALTIME_TRANSCRIPTION_MODEL),
  ...
  test("places gpt-live-transcribe only at the nested transcription-model path", () => {
    ...
    expect(JSON.stringify(session).match(/gpt-live-transcribe/g)).toEqual([
      REALTIME_TRANSCRIPTION_MODEL,
    ]);
    ...
    expect(
      (session.audio as { input: { transcription: Record<string, unknown> } })
        .input.transcription,
  ...
  test("does not send both language and languages when context is present", () => {
    ...
    expect(nestedTranscriptionModel(event.session)).toBe(
      REALTIME_TRANSCRIPTION_MODEL,
    );
  });
});
