...
export type VoicePermissionState =
  "unknown" | "checking" | "granted" | "denied" | "blocked";
...
export function failureStateFor(code: VoiceErrorCode): VoiceState {
  if (code === "MIC_PERMISSION_DENIED" || code === "MIC_PERMISSION_BLOCKED")
    return "permission_denied";
  if (code.startsWith("AUDIO_") || code === "RESAMPLE_FAILED")
    return "capture_failed";
  if (
    code.startsWith("REALTIME_") ||
...
export class VoiceStateMachine {
  ...
  resetForStart(): VoiceSnapshot {
    if (
      ...
      !isFailureState(this.value.state) &&
      this.value.state !== "review" &&
  ...
  transition(
    ...
    if (!transitions[this.value.state].includes(next)) {
      throw new Error(
        `Illegal voice transition: ${this.value.state} -> ${next}.`,
      );
    }
