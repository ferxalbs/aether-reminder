...
function candidate(): LegacySharedCaptureAsset {
  return {
    ...
    taskId: "task-asset",
    assetRef:
      "file:///app-group/capture-assets/committed/capture-asset/source.png",
  };
}
...
describe("CaptureAssetAuthorityMigrator", () => {
  test("updates the source reference only after host-private adoption succeeds", async () => {
    ...
    const migrator = new CaptureAssetAuthorityMigrator(
      repository,
      async (_assetRef, captureId) =>
        `file:///host-private/task-sources/${captureId}/source.png`,
    );

    expect(await migrator.migrateBatch()).toEqual({
      examined: 1,
      migrated: 1,
      failed: 0,
    });
  ...
  test("converges when the private copy exists after a crash before SQLite acknowledgement", async () => {
    ...
    const repository = {
      async listLegacySharedImageAssets() {
        return databaseAcknowledged ? [] : [candidate()];
      },
      async replaceImageAssetRef() {
        replaceAttempts += 1;
        if (replaceAttempts === 1)
          throw new Error("crash before reference update");
