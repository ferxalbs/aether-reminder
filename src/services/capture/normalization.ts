  ...
  switch (part.kind) {
    ...
    case "image": {
      ...
      if (!SUPPORTED_CAPTURE_IMAGE_MIMES.has(mimeType)) {
        throw new CaptureError(
          "unsupported_mime",
          `Unsupported image MIME type: ${mimeType || "empty"}.`,
        );
      }
      ...
      if (
        ...
        (!Number.isSafeInteger(part.sizeBytes) ||
          part.sizeBytes < 0 ||
          part.sizeBytes > MAX_CAPTURE_IMAGE_BYTES)
      ) {
        throw new CaptureError(
          "oversized_payload",
          "The captured image is too large.",
        );
      }
      return {
        kind: "image",
        ...
        ...(safeDisplayName(part.displayName)
          ? { displayName: safeDisplayName(part.displayName) }
          : {}),
      };
    }
    default:
      throw new CaptureError(
        "unsupported_part",
        "This capture payload is not supported.",
      );
  }
}

export function normalizeCaptureEnvelope(
  ...
  if (!Array.isArray(envelope.parts) || envelope.parts.length === 0) {
    throw new CaptureError(
      "empty_payload",
      "Capture has no supported content.",
    );
  }
  ...
  if (parts.filter((part) => part.kind === "image").length > 1) {
    throw new CaptureError(
      "unsupported_part",
      "Only one image can be captured at a time.",
    );
  }
  if (!Number.isFinite(Date.parse(envelope.createdAt))) {
    throw new CaptureError(
      "invalid_envelope",
      "Capture creation time is invalid.",
    );
  }
  return {
    ...envelope,
    id: assertStableToken(envelope.id, "Capture ID"),
    idempotencyKey: assertStableToken(
