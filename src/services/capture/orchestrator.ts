...
import { CaptureError, type CaptureDraft, type CaptureEnvelope, type CaptureEventName } from './types';

export interface CaptureAssetManager {
  adopt(source: Extract<CaptureSource, { kind: 'image' }>, captureId: string): Promise<Extract<CaptureSource, { kind: 'image' }>>;
  discardCapture(captureId: string): Promise<void>;
}

export interface CaptureEventSink {
  record(name: CaptureEventName, envelope: CaptureEnvelope, metadata?: Record<string, string>): Promise<void>;
}

export interface CaptureInvalidationSink {
  taskCommitted(task: Task, options: { attention: boolean; reliability: boolean }): Promise<void>;
}

const noAssets: CaptureAssetManager = {
  async adopt(source) { return source; },
...
export class CaptureOrchestrator {
  ...
  prepare(rawEnvelope: CaptureEnvelope, now = new Date()): CaptureDraft {
    ...
    const url = sources.find((source): source is Extract<CaptureSource, { kind: 'url' }> => source.kind === 'url');
    if (url) {
      return {
    ...
    throw new CaptureError('domain_validation', 'Enter what AETHER should remember for this image.');
  }
  ...
  async commit(
    ...
    if (!title) throw new CaptureError('domain_validation', 'A reminder title is required.');

    const sources: CaptureSource[] = [];
    for (const source of draft.sources) {
      sources.push(source.kind === 'image' ? await this.assets.adopt(source, envelope.id) : source);
    }
