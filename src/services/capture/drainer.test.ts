...
describe("CaptureInboxDrainer", () => {
  test("converges after task commit succeeds but inbox acknowledgement crashes", async () => {
    ...
    const second = await new CaptureInboxDrainer(
      restartedInbox,
      orchestrator,
    ).drain(new Date("2026-08-12T12:01:00.000Z"));
  ...
  test("classifies failures and lets a later valid capture proceed", async () => {
    ...
    const result = await new CaptureInboxDrainer(inbox, fakeOrchestrator, {
      onTerminalFailure: async (captureId) => {
        cleaned.push(captureId);
      },
    }).drain();
