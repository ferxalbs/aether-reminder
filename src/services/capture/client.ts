...
export async function initializeCaptureInbox(): Promise<CaptureInboxRepository> {
  ...
  initializing = (async () => {
    ...
    const directory = getCaptureSharedDirectory() ?? SQLite.defaultDatabaseDirectory;
    const native = await SQLite.openDatabaseAsync(CAPTURE_INBOX_DATABASE_NAME, {}, directory);
...
export function __setCaptureInboxForTests(value: CaptureInboxRepository | null): void {
  inbox = value;
  database = null;
  initializing = null;
}
