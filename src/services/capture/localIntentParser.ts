...
export function parseLocalReminderInput(
  ...
  const priorityMatch = working.match(
    /(?:^|\s)!(high|medium|low|alta|media|baja)(?=\s|$)/i,
  );
  if (priorityMatch) {
  ...
  if (relativeMatch) {
    ...
    const contextual24HourTime = working.match(
      /\b(?:at|a\s+las?)\s+(\d{1,2}):(\d{2})\b/i,
    );
    const standalone24HourTime = working.match(
      /\b([01]?\d|2[0-3]):([0-5]\d)\b/,
    );
    const timeMatch =
      meridiemTime ?? contextual24HourTime ?? standalone24HourTime;
