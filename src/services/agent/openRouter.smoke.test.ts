...
smoke('manual OpenRouter agent smoke path', () => {
  test('streams text, executes a task tool, and receives model continuation', async () => {
    ...
    for await (const event of runtime.run({
      message: 'Create a task titled Smoke test task for tomorrow, then tell me what you did.',
      context: {
        surface: 'home',
    ...
    const toolIndex = events.findIndex(
      (event) => event.type === 'tool.completed' && event.toolId === 'tasks.create'
    );
    const responseIndex = events.findIndex((event) => event.type === 'response.completed');
