...
export const tasksCreate: AgentTool<{
  ...
  version: "1",
  description:
    "Create a task. Use YYYY-MM-DD for dueDate. Prefer tomorrow as resolved local date when user says tomorrow.",
  ...
  async execute(input, ctx): Promise<ToolResult> {
    ...
    try {
      ...
      return {
        ok: false,
        error: e instanceof Error ? e.message : "Invalid due date",
      };
    }
    ...
    return {
      ok: true,
      data: {
        task: { id: value.id, title: value.title, dueDate: value.dueDate },
      },
...
export const tasksComplete: AgentTool<{ id: string }> = {
  ...
  async execute(input, ctx): Promise<ToolResult> {
    ...
    if (!id) return { ok: false, error: "id is required" };
    try {
      const { value, receipt } = await ctx.commands.completeTask(
        id,
        ctx.eventSource,
      );
      return {
...
export const tasksReopen: AgentTool<{ id: string }> = {
  ...
  async execute(input, ctx): Promise<ToolResult> {
    ...
    if (!id) return { ok: false, error: "id is required" };
    try {
      const { value, receipt } = await ctx.commands.reopenTask(
        id,
        ctx.eventSource,
      );
      return {
...
export const tasksDelete: AgentTool<{ id?: string; ids?: string[] }> = {
  ...
  async execute(input, ctx): Promise<ToolResult> {
    ...
    if (unique.length === 0)
      return { ok: false, error: "id or ids is required" };
