...
describe('notification reconciliation', () => {
  test('incremental mode processes dirty reminders without scanning clean rows', async () => {
    ...
    const result = await service.reconcile({ mode: 'incremental', reason: 'test' });
  ...
  test('full mode removes orphan and duplicate native schedules', async () => {
    ...
    const adapter: LocalNotificationAdapter = {
      list: async () => native,
      schedule: async () => { throw new Error('valid schedule must not be recreated'); },
    ...
    const result = await service.reconcile({ mode: 'full', reason: 'cold-start' });
  ...
  test('continues after blocked native scheduling failure', async () => {
    ...
    const adapter: LocalNotificationAdapter = {
      ...
      schedule: async ({ reminderId }) => {
        if (reminderId === first.id) {
          throw new NotificationError('PERMISSION_DENIED', 'Notifications are disabled.');
        }
        return `native-${reminderId}`;
      },
      cancel: async () => undefined,
    ...
    expect((await repos.reminders.getById(first.id))?.projectionState).toBe('blocked');
    expect((await repos.reminders.getById(second.id))?.projectionState).toBe('scheduled');
    await db.closeAsync?.();
  });
});
