...
describe("handleNotificationActionResponse", () => {
  test("completes through AetherCommandExecutor", async () => {
    ...
    expect(
      await handleNotificationActionResponse(
        response(NOTIFICATION_ACTION_COMPLETE),
        harness.value,
      ),
    ).toBe(true);
    expect(harness.completeTask).toHaveBeenCalledWith(
      "task-1",
      "notification_action",
    );
  });
  ...
  test("snoozes exactly ten minutes using local calendar values", async () => {
    ...
    await handleNotificationActionResponse(
      response(NOTIFICATION_ACTION_SNOOZE),
      harness.value,
      now,
    );
    const target = new Date(now.getTime() + 10 * 60_000);
    expect(harness.rescheduleReminder).toHaveBeenCalledWith(
      "reminder-1",
      {
  ...
  test("moves the reminder to tomorrow while preserving its time", async () => {
    ...
    await handleNotificationActionResponse(
      response(NOTIFICATION_ACTION_TOMORROW),
      harness.value,
      now,
    );
    expect(harness.rescheduleReminder).toHaveBeenCalledWith(
  ...
  test("uses fixed reminder timezone and persists one snooze target across retries", async () => {
    ...
    await expect(
      handleNotificationActionResponse(actionResponse, actionCore, firstNow),
    ...
    expect(rescheduleReminder).toHaveBeenNthCalledWith(
      1,
      "reminder-1",
      {
        scheduledDate: "2026-08-10",
  ...
  test("does not repeat completed action after process-local replay", async () => {
    ...
    const actionCore = {
      ...
      commands: {
        completeTask,
        rescheduleReminder: mock(async () => ({ value: storedReminder })),
      },
    } as unknown as AetherCore;
