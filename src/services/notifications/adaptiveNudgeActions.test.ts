  ...
  return {
    ...
    notification: {
      request: {
        ...
        content: {
          data: {
            reminderId: "adaptive-action-nudge",
...
describe("adaptive notification actions", () => {
  test("duplicate cold/action responses record one local behavior event", async () => {
    ...
    expect(await handleNotificationActionResponse(response(), core, now)).toBe(
      true,
    );
    expect(await handleNotificationActionResponse(response(), core, now)).toBe(
      false,
    ...
    expect(
      (await repos.reminders.getById("adaptive-action-nudge"))?.scheduledTime,
    ).toBe("09:30");
    await db.closeAsync?.();
  });
});
