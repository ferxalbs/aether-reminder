...
describe('notification bootstrap', () => {
  ...
  test('surfaces partial reconciliation as a retryable error instead of silently succeeding', async () => {
    const client = {
      ...
      reconcile: async () => ({
        ...
        failures: [{
          ...
          error: new NotificationError('PERMISSION_DENIED', 'Notifications are disabled.'),
        }],
      }),
    };
    ...
    await expect(syncLocalNotifications(client)).rejects.toMatchObject({
      ...
      retryable: true,
      message: 'One reminder could not be synchronized with device notifications. Try again.',
    });
  });
