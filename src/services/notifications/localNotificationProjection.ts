  ...
  async list() {
    ...
    return (await Notifications.getAllScheduledNotificationsAsync()).map(
      (item) => ({
        ...
        reminderId:
          typeof item.content.data?.reminderId === "string"
...
export function resolveReminderNotificationDate(
  ...
  if (!reminder.scheduledDate) {
    throw new NotificationError(
      "INVALID_TRIGGER",
      "Reminder has no scheduled date.",
    );
  }
  const timezone =
  ...
  if (!timezone) {
    ...
    const [hour, minute] = (reminder.scheduledTime ?? "09:00")
      .split(":")
      .map(Number);
...
export class LocalNotificationProjection {
  ...
  getCapabilities(): Promise<NotificationCapabilities> {
    return (
      ...
      Promise.resolve(defaultNotificationCapabilities())
    );
  }
  ...
  async reconcile(): Promise<{
    ...
    const { NotificationReconciliationService } =
      await import("./notificationReconciliation");
  ...
  async project(reminder: Reminder): Promise<ProjectionResult> {
    ...
    try {
      const claimed = await this.reminders.recordProjectionAttempt(
        reminder.id,
        revision,
      );
      ...
      if (reminder.nativeNotificationId) {
        try {
          ...
          if (cancellationError.code !== "NATIVE_NOTIFICATION_MISSING")
  ...
  try {
    ...
    Notifications.setNotificationHandler({
      handleNotification: async (notification) => ({
        ...
        shouldShowList: true,
        shouldPlaySound:
          notification.request.content.data?.presentationPolicy !== "gentle",
