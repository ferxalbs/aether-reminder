...
import type { SqlBindParams, SqlDatabase, SqlRunResult, SqlValue } from './types';

function normalizeParams(params?: SqlBindParams): SqlValue[] | Record<string, SqlValue> | undefined {
    ...
    async runAsync(source: string, params?: SqlBindParams): Promise<SqlRunResult> {
      ...
      const result =
        bound === undefined
          ...
          : Array.isArray(bound)
            ? stmt.run(...bound)
            : stmt.run(bound as Record<string, string | number | null | Uint8Array | boolean>);
      return {
    ...
    async getFirstAsync<T>(source: string, params?: SqlBindParams): Promise<T | null> {
      ...
      const row =
        bound === undefined
          ...
          : Array.isArray(bound)
            ? stmt.get(...bound)
            : stmt.get(bound as Record<string, string | number | null | Uint8Array | boolean>);
    ...
    async getAllAsync<T>(source: string, params?: SqlBindParams): Promise<T[]> {
      ...
      const rows =
        bound === undefined
          ...
          : Array.isArray(bound)
            ? stmt.all(...bound)
            : stmt.all(bound as Record<string, string | number | null | Uint8Array | boolean>);
      return rows as T[];
    },
