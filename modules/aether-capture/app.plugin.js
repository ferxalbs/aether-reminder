...
function updateTargetBuildSettings(project, targetUuid, settings) {
  ...
  if (!target || !configurationList) {
    throw new Error('AetherCapture: Share Extension build configurations are unavailable.');
  }
  ...
  for (const configuration of configurationList.buildConfigurations) {
    ...
    if (!buildConfiguration?.buildSettings) {
      throw new Error('AetherCapture: Share Extension build settings are unavailable.');
    }
...
function withAetherCaptureApple(config) {
  ...
  return withXcodeProject(config, (mod) => {
    ...
    const sourceRoot = path.join(projectRoot, 'targets', 'aether-share-extension');
    ...
    for (const filename of [
      ...
      copyFile(path.join(sourceRoot, filename), path.join(extensionRoot, filename));
    }
    for (const filename of ['CaptureInboxWriter.swift', 'CaptureAppIntents.swift']) {
      copyFile(path.join(sourceRoot, filename), path.join(appCaptureRoot, filename));
    }
    ...
    if (!extensionTarget) {
      ...
      project.addBuildPhase([], 'PBXResourcesBuildPhase', 'Resources', extensionTargetUuid);
    } else {
      extensionTargetUuid = project.findTargetKey(EXTENSION_NAME);
    }
    if (!extensionTargetUuid) throw new Error('AetherCapture: Share Extension target UUID is unavailable.');

    for (const filename of ['ShareViewController.swift', 'CaptureInboxWriter.swift']) {
      addSourceIfMissing(
        project,
...
module.exports = createRunOncePlugin(withAetherCapture, 'aether-capture', '1.0.0');
