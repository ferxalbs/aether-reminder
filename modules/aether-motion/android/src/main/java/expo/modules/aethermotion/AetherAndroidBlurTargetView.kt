...
/**
 * Android-only counterpart to Expo Blur's target view. It owns the Dimezis
 * BlurTarget as a sibling of the diagnostic blur view, so the capture graph is
 * non-recursive and the existing route-content topology remains explicit.
 */
class AetherAndroidBlurTargetView(context: Context, appContext: AppContext) : ExpoView(context, appContext) {
  internal val blurTargetView: BlurTarget = UIManagerCompatibleAetherBlurTarget(context)
