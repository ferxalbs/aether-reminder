...
/**
 * Android tint mapping kept equivalent to expo-blur's current Dimezis path.
 * Tint is intentionally not a diagnostic variable; only scaleFactor varies.
 */
internal enum class AetherBlurTint(val value: String) {
  DEFAULT("default"),
  ...
  fun toBlurEffect(blurRadius: Float): Int {
    return when (this) {
      ...
      PROMINENT,
      DEFAULT,
      SYSTEM_MATERIAL -> DEFAULT.toColorInt(blurRadius)
  ...
  private fun toColorInt(blurRadius: Float): Int {
    ...
    return when (this) {
      ...
      SYSTEM_CHROME_MATERIAL_DARK -> ((255 * intensity * 0.75).toInt() shl 24)
      else -> ((255 * intensity * 0.44).toInt() shl 24) + (255 shl 16) + (255 shl 8) + 255
    }
  }

  companion object {
    fun from(value: String): AetherBlurTint =
